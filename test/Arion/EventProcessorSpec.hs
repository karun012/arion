...
spec = do
    describe "Event Processor" $ do
        it "responds to a Modified event on a test file by creating commands to run tests" $ do
            ...
            let expectedCommands = [Echo "mydir/ModuleASpec.hs changed", CabalExec $ RunHaskell "src" "mydir/ModuleASpec.hs"]
        ...
        it "responds to a Modified event on a source file by creating commands to run the associated tests" $ do
            ...
            let sourceFolder = "src"
            let expectedCommands = [Echo "src/ModuleA.hs changed", CabalExec $ RunHaskell "src" "test/ModuleASpec.hs",
                                       CabalExec $ RunHaskell "src" "test/ModuleBSpec.hs"]
        ...
        it "does not ignore lhs files" $ do
            ...
            let sourceFolder = "src"
            let expectedCommands = [Echo "mydir/ModuleASpec.lhs changed", CabalExec $ RunHaskell "src" "mydir/ModuleASpec.lhs"]
